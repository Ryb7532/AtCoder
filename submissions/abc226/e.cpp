...
struct UnionFind {
  ...
  int root(int a) {
    if (par[a].first < 0)
      return a;
    return par[a].first = root(par[a].first);
  }
  int size(int a) { return -par[root(a)].first; }
  void unite(int a, int b) {
    ...
    if (par[ra]>par[rb])
      swap(ra, rb);
    par[ra].first += par[rb].first;
    par[ra].second += par[rb].second + 1;
    par[rb] = {ra, 0};
    return;
  }
  ...
  pair<int,int> &operator[](const int i) { return par[i]; }
};
