...
int main() {
  ...
  auto h = [](OperatorMonoid a, OperatorMonoid b) { return a^b; };
  RUQ<Monoid, OperatorMonoid> seg(N, f, g, h, Monoid(true, true), false);
  rep(i,N) {
    bool f = S[i] == '1';
    if (i&1)
      f = !f;
    seg.set(i, Monoid(f, !f));
  }
  seg.build();
  rep(Q) {
    int t,l,r;
