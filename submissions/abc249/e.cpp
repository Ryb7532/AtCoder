...
int main() {
  int N;
  cin >> N >> mod;
  vector<vector<vector<mint>>> dp(N, vector<vector<mint>>(N+5, vector<mint>(2)));
  dp[0][2][0] = 26;
  rep(i,N-1) {
    rep(j,N) {
      if (j+2 < N)
        dp[i+1][j+2][0] += dp[i][j][0] * 25 + dp[i][j][1] * 25;
      mint carry_over = 0;
      int t = 10;
      for (int k=0; i+1-t+1 >= 0; k++, t*=10) {
        if (j-k >= 0)
          carry_over += dp[i+1-t+1][j-k][0];
      }
      dp[i+1][j][1] += dp[i][j][0] + dp[i][j][1] - carry_over;
      if (j+1 < N)
        dp[i+1][j+1][1] += carry_over;
    }
  }
  mint res;
  rep(j,N) {
