...
int main() {
  ...
  auto f = [](Monoid a, Monoid b) {return Monoid(a.first + b.first, min(a.second, b.second));};
  RMQ<Monoid> seg(N, f, Monoid(0, N));
  rep(i,N) {
    scan(A[i]);
  }
  rep(i,N) {
  ...
  rep(i,N) {
    if (B[i] != 1)
      seg.set(i, Monoid(A[i], i));
    else
      seg.set(i, Monoid(A[i], N));
  }
  seg.build();
  scan(Q);
  rep(_,Q) {
    ...
    if (t == 3) {
      ...
      ll res = 0;
      while (l < r) {
        auto [sum, next_l] = seg.query(l,r);
        if (next_l < r) {
          res += seg.query(l,next_l).first;
          res = max(res + A[next_l], res * B[next_l]);
        } else {
          res += sum;
        }
        l = next_l + 1;
