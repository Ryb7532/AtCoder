...
int main() {
  ...
  auto f = [](Monoid a, Monoid b, int l, int r) {
    return (Monoid){a.first + b.first, 0};
  };
  auto g = [](Monoid a, OperatorMonoid b, int l) {
    return (Monoid){a.first + b.first * (1-a.second), 1 - (1-a.second) * (1-b.second)};
  };
  auto h = [](OperatorMonoid a, OperatorMonoid b) {
    return (OperatorMonoid){a.first + b.first * (1-a.second), 1 - (1-a.second) * (1-b.second)};
  };
  RUQ<Monoid, OperatorMonoid> seg(N, f, g, h, (Monoid){0,0}, (OperatorMonoid){0,0});
  seg.build();
  rrep(i,M) {
    mint p = (mint) 1 / (R[i]-L[i]);
    seg.update(L[i], R[i], (OperatorMonoid){X[i]*p, p});
  }
  rep(i,N) {
    seg.update(i, i+1, (OperatorMonoid){A[i], 1});
  }
  rep(i,N) {
    cout << seg[i].first << " ";
  }
