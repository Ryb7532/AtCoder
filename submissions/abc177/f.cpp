...
int main() {
  int H,W;
  cin >> H >> W;
  auto f = [](int a, int b, int la, int lb) {
    return min(a,b);
  };
  ...
  const int INF = 1e9;
  RUQ<int> seg(W+1, f, g, h, INF, 0);
  RUQ<int,int> right(W+1, f, g, h, INF, 0);
  rep(i,W) {
    seg.set(i,0);
    right.set(i,i);
  }
  ...
  rep(i,H) {
    ...
    int l = right.find_last(W+1,cl)+1;
    int r = right.find_last(W+1,cr);
    int v = b-r;
    if (b == W) {
      v = INF;
      b = INF;
    ...
    seg.update(l,r+1,v);
    right.update(l,r+1,b);
    int res = seg.query(0,W);
    if (res >= INF)
      print(-1);
    else
      print(res+i+1);
