...
int main() {
  ...
  sort(all(id), [&](int a, int b){ return C[a] < C[b]; });
  using Monoid = int;
  using OperatorMonoid = Monoid;
  auto f = [](Monoid a, Monoid b, int l, int r) { return a+b; };
  auto g = [](Monoid a, OperatorMonoid b, int len) { return b*len; };
  auto h = [](OperatorMonoid a, OperatorMonoid b) { return max(a, b); };
  RUQ<Monoid, OperatorMonoid> seg(N-1, f, g, h, 0, 0);
  seg.build();
  ll cost = 0;
  fore(i,id) {
    cost += C[i];
    int cnt = seg.query(L[i], R[i]);
    cost += C[i] * (R[i]-L[i]-cnt);
